...
assayData - contains the expression of targets (either probes or genes) for each sample 
information. 
...
```{r setDefaults,  eval = FALSE, show = FALSE}
...
sData(LowReads)["Raw"]
head(QCResults[rownames(sData(LowReads)),])
...
head(QCResults[rownames(sData(LowSaturation)),])
```

##### Set QC flags Update cutoffs
If needed, you can set the threshold for sequencing cutoff.
...
exprs(negativeControlSubset(lowNegatives))[1:5, 1:5]
...
##### Set Segment QC flags Update cutoffs
