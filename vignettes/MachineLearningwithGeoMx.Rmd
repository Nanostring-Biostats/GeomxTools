...
The count of each class is similar, no imbalance.
```{r}
...
### Input Parameters for ML Function
...
#### 1. Naive Bayes (nb)
The Naive Bayes Classifier is a probabilistic classifier that assumes independence between features.
Parameters: 
1. Laplace Correction (laplace)
2. Distribution Type (usekernel)
3. Bandwidth Adjustment (adjust)
...
Parameters: 
1. Boosting Iterations (nrounds)
2. Max Tree Depth (max_depth)
3. Shrinkage (eta)
4. Minimum Loss Reduction (gamma)
5. Subsample Percentage (subsample)
6. Subsample Ratio of Columns (colsample_bytree)
7. Fraction of Trees Dropped (rate_drop)
8. Probability of Skipping Drop-out (skip-drop)
9. Minimum Sum of Instance Weight (min_child_weight)

#### 4. Random Forest (rf)
The Random Forest Classification Model...
Parameters: 
1. Starting Value of Number of Predictors Randomly Sampled at Each Split (mtryStart)
2. Number of Trees Used at The Tuning Step (ntreeTry)
3. Number By Which mtry is Changed at Every Iteration (stepFactor)
4. Required Improvement In OOB Error for Search to Continue (improve)
5. Print Progress of Search (plot)
6. Run Forest Using Optimal mtry Found (doBest)
...
Implement parameter checking in mlfunction, error handling (prioritize checking for incorrect/unused parameters). Implement Cross Validation Function over entire dataset (don't split into train and test). More models, more complex models, more control over model parameters. More testing with different datasets.
