...
For the target QC step, we identify proteins with no useful signal using this figure. 
...
```
  ...
  -	Tyrosine Hydroxylase (orange) hovers around background in all segments and should probably be excluded from analysis. 
...
To remove proteins we will first pick a protein where all of the proteins to the left of it should be removed from analysis due to low signal. Here we will pick **P62** since the protein to the right, **HSC70**, has meaningful signal. And replot the figure without the super low proteins. 
...
After filtering targets, we move onto normalization. There are many types of normalization and we have two built in figures types to help decide what is the best method for the dataset. 
...
```{r IgG concordance}
fig <- plotConcordance(object = proteinData, targetList = igg.names, plotFactor = "Segment_Type")
fig()
fig <- plotConcordance(object = proteinData, targetList = igg.names, plotFactor = "Tissue")
fig()
```
...
fig <- plotNormFactorConcordance(object = proteinData, plotFactor = "Segment_Type",
...
This divergence of area and nuclei vs IgGs and HKs is common which is why Negative or HK normalization is recommended. The area and nuclei plots are good QC metrics to look for outliers or additionally can help you potentially ID some preferential bias in a study design.

After choosing a normalization technique from these plots, we normalize the data. Area and nuclei normalization are not native functions in GeomxTools, if you decide on normalizing by those factors you will need to do that separately. Quantile normalization is also available if HK or negative normalization are not preferred. 
