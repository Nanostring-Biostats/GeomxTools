...
```
``` {r backgroundLibraries, echo = FALSE, eval = TRUE}
...
We will start by loading the data. the `readNanoStringGeoMxSet`**LINK NEEDED** function loads in all the necessary components, returning a structured R object that collates this information. The key data files are:
...
The section below will load data based on each being in separate folders within one central location. It will search through the folder to identify only the files with the appropriate extensions.
...
![](./GeomxSetStructure.png){width=60%}
...
* To learn more about other accessor functions type: `?GeomxTools::sData`

Both eSets and GeoMxSets use `esApply` to extend [`apply`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/apply) function to such objects. In addition to the `esApply` function defined by the `ExpressionSet` class, we have built the `assayDataApply` function to allow you to select which expression matrix `elt` you wish to use with the `esApply` function. We will demonstrate the use of these functions below. 

## Quality Control & Pre-processing

The steps below encompass the standard pre-processing workflow for scRNA-seq data in Seurat. These represent the selection and filtration of cells based on QC metrics, data normalization and scaling, and the detection of highly variable features

First we will shift counts with 0 detection to 1 to prevent `-Inf` in downstream transformations. This uses the `shiftCountsOne` function.
...
A common method for exploring differences between groups of segments or patients is to perform differential expression analysis. While advanced methods for modeling the underlying count distribution are being actively developed, a common way to perform differential expression analysis with GeoMx data is to use a linear mixed-effect model (LMM). LMMs allow the user to account for the fact that we sample multiple times within a slide, rather than assuming that each AOI is independently sampled. These models are broken into 2 components: fixed effects - which represent the hypothesis you are testing, and random effects, which are variables estimating slide-specific variation.

For studies where you compare features within slides, we use a complex model to account for shared information shown below. For studies analyzing results between slides we can use a more simplistic model (see the section on between slide analysis). For this analysis we use the package `lme4` to model the data and `lmerTest` to estimate the significance of interactions. For more information about mixed effect models please see [the original lme4 documentation](https://cran.r-project.org/web/packages/lme4/vignettes/lmer.pdf).

#### Within Slide Analysis: Glomeruli vs Tubules

The study we have collected allows us to ask what genes are specifically expressed within unique morphological structures within the kidney, and identify marker genes for either glomeruli or tubules. Because we are comparing structures found within the same slide we will use a complex model defined as the following code: `log2(Gene) ~ Region + (1 + Slide | Slide)`. Region is what we want to test against, and we use the second expression to account for variation within and across slides. This is a model with random effects for both slope (`(1 + slide |`) and intercept (`| Slide)`). In the next example below we change the formula to be less complex.

In this analysis we save log~2~ fold change estimates and P-values between the regions by accessing the coefficients table of the model using `coef(summary(...))`. If you want additional information about the t-statistics simply change the code below to save all of the coefficients from the lmm by adjusting the reference to `coef(...)[2, ]` instead of `coef(...)[2, c(1,5)]`. We also used a [Benjamini-Hochberg](http://www.math.tau.ac.il/~ybenja/MyPapers/benjamini_hochberg1995.pdf) multiple-test correction for the p-values, but different methods can be used by typing `?p.adjust` to read through options.
