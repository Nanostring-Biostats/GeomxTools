...
author: "Jason Reeves, Prajan Divaker, David Henderson, Nicole Ortogero, Zhi Yang, Rona Vitancol, Maddy Griswold"
date: "`r Sys.Date()`"
output: 
...
### DCC files, PKC files and annotation file
...
After loading we shift counts with 0 detection to 1 so that during downstream processing and transformations we do not end up with `-Inf` values in our tables. This uses the `shiftCountsOne` function, so that no additional coding is necessary.
...
With the `demoData`, you have just created your first *GeoMxSet Object*. These objects are how we manage expression data, annotations, and meta-data about the probes and study throughout an experiment when working with `GeomxTools`. They are based on `ExpressionSet` objects, and have many similar functions as those described [on Bioconductor](https://www.bioconductor.org/packages/release/bioc/vignettes/Biobase/inst/doc/ExpressionSetIntroduction.pdf). All of this information is linked and stored together, and there are a few key ways to access the data now that we have created a GeoMxSet Object. Below is a schematic overview of some of the data and functions we will use in the rest of the vignette.

![GeoMxSet Object Overview](./GeomxSetStructure.png){width=60%}

As is shown above the GeoMxSet object consists of 3 or more elements.

* Expression count matrices are the primary data that will be used for analysis - accessed with `exprs(object)` to return the matrix.
* As we progress through the data we will add new expression matrices to expression slots, or elements (elt), which can be accessed with `assayDataElement(object, elt = ...)`
* Segment & Sample annotations are managed - accessed with `pData(object)`
* Probe & Target annotations & meta-data - accessed with `fData(object)`

The package uses `esApply` to load in feature or phenotypic data for use with a standard matrix `apply` function. We will demonstrate it's use several times below. In addition to the `esApply` function defined by the `ExpressionSet` class, we have also added the `assayDataApply` function to allow you to select which expression matrix you wish to use with the `esApply` function.
...
btm_row <- plot_grid(plt2, plt3, nrow = 1, labels = c('B',''), rel_widths = c(0.4,0.6))
...
One way to visualize the clusters observed above is to calculate the coefficient of variation (CV), for each AOI ($i$) using the formula $CV = SD_i/mean_i$. After calculating we can look for genes with high CVs which should have large differences across the various segments that we have profiles. This is an unbiased way of showing the genes driving structure within the dataset.


``` {r CVheatmap, eval = TRUE, fig.width = 8, fig.height = 6.5}
calc_CV <- function(x) {
  sd(log2(x)) / mean(log2(x))
}
...
Use the following code to calculate a LMM looking for variation on between patients, for example between diabetic kidney disease and normal patients within the glomeruli. This is generally a more common scenario for analysis. Note that this is set up without a random slope variable, just a random intercept. [Update TEXT HERE]
...
Going back to the test for distinctions between glomeruli & tubules, the table produced captures 2 of the key features for DE. It saved the log~2~ fold change value, called `Estimate` in the table, and the P-value, called `Pr(>|t|)`. If you want additional information about the t-statistics simply change the code above to save all of the coefficients from the lmm by adjusting the reference to `coef(...)[2, ]` instead of `coef(...)[2, c(1,5)]`. We also used a [Benjamini-Hochberg](http://www.math.tau.ac.il/~ybenja/MyPapers/benjamini_hochberg1995.pdf) multiple-test correction for the p-values, but different methods can be used by typing `?p.adjust` to read through options
...
#### Graphing a Volcano Plot

Let's code a simple `ggplot2` graph for a volcano plot, which will color based on various cuts in the significance threshold and label genes the top 50 genes based on P-value on either side of the volcano plot. 

``` {r volcanoPlot, fig.width = 10, fig.height = 8, warning = FALSE, message = FALSE}
...
#### Plotting individual gene expression values
...
ggplot(pData(target_demoData),
       aes(x = region, fill = region,
           y = assayDataElement(target_demoData['KRT8', ], elt = 'q_norm'))) +
  geom_boxplot() +
  labs(y = 'PODXL Expression') +
  scale_y_continuous(trans = 'log2') +
  theme_bw()

# show expression of PODXL vs CLU
ggplot(pData(target_demoData),
       aes(x = assayDataElement(target_demoData['KRT8', ], elt = 'q_norm'),
           y = assayDataElement(target_demoData['PODXL', ], elt = 'q_norm'),
           color = region, shape = class)) +
  geom_vline(xintercept = max(assayDataElement(target_demoData['KRT8', glom], elt = 'q_norm')),
             lty = 'dashed', col = 'darkgray') +
  geom_hline(yintercept = max(assayDataElement(target_demoData['PODXL', !glom], elt = 'q_norm')),
             lty = 'dashed', col = 'darkgray') +
  geom_point(size = 3) +
  theme_bw() +
  scale_x_continuous(trans = 'log2') + scale_y_continuous(trans = 'log2') +
...
Here we can also compare the expression of these genes to those in other databases. The human protein atlas is a key resource for understanding protein localization. In each case the qualified expression by structure is concordant with these results:
