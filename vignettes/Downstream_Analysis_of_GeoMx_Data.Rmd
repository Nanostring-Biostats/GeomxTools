...
calc_LOQ <- function(object = NULL,
  ...
  esBy(negativeObject, GROUP = "Module", 
       function(x) {assayDataApply(x, MARGIN = 2, elt = "exprs",
                                   FUN = function(y) {
                                     pmax(ngeoMean(y) * ngeoSD(y) ^ cutoff,
                                          minLOQ)})})
}

pData(demoData)[['LOQ']] <- calc_LOQ(demoData)

# LOQ_cols <- paste0('LOQ_', modules)
# pData(demoData)[, LOQ_cols] <- calc_LOQ(demoData)
...
First we will determine the number of genes detected in each segment and across the dataset. We will also show you how to examine the detection rates of genes of interest, and performance for each segment and gene.

```{r geneDetectionRate, eval = TRUE}
pData(target_demoData)[['LOQ']] <- pData(demoData)[['LOQ']] # unclear if needed now
LOQ_Mat <- c()
for(i in modules) {
  ind <- fData(target_demoData)$Module == i
...
First let's go ahead and remove segments for which we have very low signal. These will be segments in which only small fraction of the panel is detected above LOQ. For this example we will remove segments with less than 10% of the panel detected. 

```{r segHistogram, fig.wide = TRUE}
# graph histogram of different cutpoints (1%, 5%, 10%, 15%)
ggplot(pData(target_demoData), 
       aes(x = GenesDetected / nrow(exprs(demoData)) * 100,
           fill = region)) +
  geom_histogram(bins = 30) +
  geom_vline(xintercept = c(1,5,10), lty = 'dashed', lwd = 1,
             color = c('red3','orange2','yellow2')) +
  theme_bw() +
  labs(x = 'Detection Rate, % Genes Detected', y = '# ROIs')
```
...
```{r filterSegments}
target_demoData <-
  target_demoData[, pData(target_demoData)$GenesDetected > .1 * nrow(demoData)]
