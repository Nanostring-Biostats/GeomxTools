...
To calculate a LMM looking for variation on a patient level, for example, between diabetic kidney disease and normal patients within the glomeruli you would use the following code. As this is a more common scenario for analysis, note that in this set up without a random slope variable, just a random intercept. [ADD TEXT HERE]

``` {r DEsimple, eval = FALSE}
lmm <- function(y) {
  coef(summary(lmer(log2(y) ~ region + (1| `slide name`))))[2, c(1,5)]
}
...
Going back to the test for distinctions between glomeruli & tubules, let's take a look at our genes of interest and what their log~2~ fold change (`Estimate`), P-value (`Pr(>|t|)`), and FDR are. We can do this by simply subsetting the results table we just generated by typing `results[goi, ]`. 
