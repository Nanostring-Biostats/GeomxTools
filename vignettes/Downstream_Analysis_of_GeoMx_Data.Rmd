...
The final results are spatially resolved unique expression datasets for every protein-coding gene (>18,000 genes) from every individual ROI/AOI segment profiled from tissue.
...
* Annotation file - useful tissue information, including the type of ROIs/AOIs profiled (ex: glomerulus vs. tubule), ROI/AOI area/nuclei count, and other tissue characteristics (ex: diseased vs. healthy)
...
tsne_out <-
  Rtsne(t(log2(assayDataElement(target_demoData , elt = "q_norm"))),
        perplexity = ncol(target_demoData)*.1)
...
Because we are exploring the relationship between expression and different tissue morphological structures, we can leverage databases to validate our findings. For example, the [Human Protein Atlas](https://www.proteinatlas.org/) is a key resource for understanding protein localization. Recognizing that there is a distinction between mRNA and protein expression, we can still find examples of top GeoMx differential expression hits that are concordant with public atlas data.
