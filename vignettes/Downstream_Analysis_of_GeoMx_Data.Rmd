...
boxplot(exprs(target_demoData)[,1:10], log = 'y', names = 1:10, xlab = 'Segment', ylab = 'Counts, Raw')
...
# run UMAP
umap_out <- umap(t(na.omit(log2(assayDataElement(target_demoData , elt = "neg_norm")))),  
                 config = custom_umap)
...
library(Rtsne)
# run UMAP
tsne_out <- Rtsne(t(na.omit(log2(assayDataElement(target_demoData , elt = "neg_norm")))), perplexity = 20)
...
# Test 1:
# Glom vs Tubules - mixed slope + intercept because the test is performed
#     comparing paired ROIs within a slide 

# between structure (glomerulus vs tubules), across samples:
lmm <- function(y) {
...
any_NA <- rowSums(is.na(assayDataElement(target_demoData[GOI, ], elt = 'neg_norm')))
GOI <- GOI[any_NA == 0]
pheatmap(assayDataElement(target_demoData[GOI, ], elt = 'neg_norm'),
         scale = 'row', 
         show_rownames = FALSE,
         show_colnames = FALSE,
         border_color = NA,
...
autoplot(target_demoData[GOI, ], type = 'heatmap-genes', elt = 'neg_norm', heatmapGroup = c('region','class'))
