...
As we can see above the 5% cutoffs only removes 2 samples while there are several more near the 10% detection range, with a bias towards glomeruli. Let's create a table to review what sample disease type is going to be impacted by each threshold:
...
Additionally, key target genes can be kept for downstream analysis by manually selecting them for inclusion in the data object. We generally recommend saving the negative control probes that we will identify using the `negativeObject` we created during LOQ calculation with `negativeControlSubset`.
...
results <- results[, c('Gene','Subset','Contrast','Estimate','Pr(>|t|)','FDR')]
```
...
Let's go back to our first statistical test comparing diseased glomeruli to tubules. We saved the LMM outputs into a table (`results`) containing two of the key features for differential expression: i) the log~2~ fold change value (`Estimate`) and ii) the P-value (`Pr(>|t|)`). Let's take a look at a few genes of interest and their fold changes, P-values, and FDR values. 
...
In the next section we'll explore the expression of SEMA3G & DSP further to demonstrate the dynamics of these targets.

## Plotting Genes of Interest

A simple and effective plot to view individual genes is the violin plot. This visualization reveals both the dynamic range and shape of the distribution for a gene target. We will plot a gene enriched within the glomeruli, SEMA3G (`UPDATE`) and a gene enriched within tubules, DSP (`UPDATE`). 
...
ggplot(pData(target_demoData),
       aes(x = region, fill = region,
           y = assayDataElement(target_demoData['SEMA3G', ],
                                elt = 'q_norm'))) +
  geom_boxplot() +
  labs(y = 'SEMA3G Expression') +
  scale_y_continuous(trans = 'log2') +
  theme_bw()

# show expression of SEMA3G vs DSP
ggplot(pData(target_demoData),
       aes(x = assayDataElement(target_demoData['DSP', ],
                                elt = 'q_norm'),
           y = assayDataElement(target_demoData['SEMA3G', ],
                                elt = 'q_norm'),
           color = region, shape = class)) +
  geom_vline(xintercept =
               max(assayDataElement(target_demoData['DSP', glom],
                                    elt = 'q_norm')),
             lty = 'dashed', col = 'darkgray') +
  geom_hline(yintercept =
               max(assayDataElement(target_demoData['SEMA3G', !glom],
                                    elt = 'q_norm')),
             lty = 'dashed', col = 'darkgray') +
  geom_point(size = 3) +
  theme_bw() +
  scale_x_continuous(trans = 'log2') + 
  scale_y_continuous(trans = 'log2') +
  labs(x = 'DSP Expression', y = 'SEMA3G Expression')
```
