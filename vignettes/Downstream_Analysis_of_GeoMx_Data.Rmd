...
ggplot(test_gr, aes(x, id = id, split = y, value = n)) +
  ...
  annotate(geom = 'text', x = 4.19, y = 70, angle = 90, size = 4,
...
``` {r qcflag histograms, fig.width = 10, fig.height = 8, fig.wide = TRUE, eval = TRUE, warning = FALSE, message = FALSE}
library(ggplot2)
library(cowplot)
...
fData(target_demoData)$DetectionRate <- fData(target_demoData)$DetectedSegments /
...
```{r plotDetectionRate, eval = TRUE, fig.small = TRUE}
# Plot detection rate:
...
``` {r, previewNF, fig.width = 8, fig.height = 6.5, fig.wide = TRUE, eval = TRUE}
...
``` {r dimReduction, eval = TRUE, fig.small = TRUE}
library(umap)
# run UMAP
