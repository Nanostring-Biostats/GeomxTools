...
``` {r sampleFlow, fig.width = 10, fig.height = 4, message = FALSE, warning = FALSE}
library(dplyr)
library(ggforce)
...
# gather the data and plot in the graph in the order: class, slide name, region, segment
...
ggplot(test_gr, aes(x, id = id, split = y, value = n)) +
  ...
  theme_classic(base_size = 13) + theme(axis.ticks.y = element_blank(), axis.line = element_blank(), axis.text.y = element_blank()) +
  scale_y_continuous(expand = expansion(0)) + scale_x_discrete(expand = expansion(0)) + labs(x = '', y = '') +
  annotate(geom = 'segment', x = 4.25, xend = 4.25, y = 20, yend = 120, lwd = 2) +
  annotate(geom = 'text', x = 4.19, y = 70, angle = 90, size = 4, hjust = 0.5, label = '100 segments')
```
...
![](./GeomxSetStructure.png){width=70%}
...
plts[[5]] <- QC_histogram(sData(demoData), 'NegGeoMean', 'region', 2) + scale_x_continuous(trans = 'log10')
plts[[6]] <- QC_histogram(sData(demoData), 'area', 'region', 5000) + scale_x_continuous(trans = 'log10')
...
```{r plotDetectionRate, eval = TRUE, out.width = '50%'}
# Plot detection rate:
...
``` {r silentSeed, eval = TRUE, echo = FALSE}
...
library(Rtsne)
# run tSNE
tsne_out <- Rtsne(t(log2(assayDataElement(target_demoData , elt = "q_norm"))), perplexity = 5)
...
pData(target_demoData)$testRegion <- factor(pData(target_demoData)$region, c('glomerulus','tubule'))
# Define statistical test
lmm <- function(gene) {
  coef(summary(lmer(log2(gene) ~ testRegion + (1 + `slide name`| `slide name`))))[2, c(1,5)]
}
# Run tests
...
pData(target_demoData)$testClass <- factor(pData(target_demoData)$class, c('normal','DKD'))
# Define statistical test
lmm <- function(gene) {
  coef(summary(lmer(log2(gene) ~ class + (1 | `slide name`))))[2, c(1,5)]
}
...
In the next section we'll explore these genes further.
