...
# Subset to target genes detected in at least 10% of the samples. Also white-list the negative control probe
...
plotr <- align_plots(plt3, plt2, align = 'v', axis = 'r')
top_row <- plot_grid(plt1, plotr[[2]], nrow = 1)
plot_grid(top_row, plt3, ncol = 1)
...
``` {r volcanoPlot, figure.width = 10, figure.height = 10}
...
ggplot(pData(target_demoData),
       aes(x = assayDataElement(target_demoData['NPHS2', ], elt = 'q_norm'),
           y = assayDataElement(target_demoData['PAX8', ], elt = 'q_norm'),
           color = region, shape = pathology)) +
  geom_point() +
  theme_bw() +
  labs(x = 'NPHS2, Q3 Counts', y = 'PAX8, Q3 Counts')
